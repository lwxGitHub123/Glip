...
class DatasetCatalog(object):
    DATASETS = {
        ...
        #  labelme_grounding
        "labelme_grounding_train": {
            "img_dir": "grounding_dataset/images/train",
            "ann_file": "grounding_dataset/annotations/labelme_grounding_train.json"
        },
        "labelme_grounding_val": {
            "img_dir": "grounding_dataset/images/val",
            "ann_file": "grounding_dataset/annotations/labelme_grounding_val.json"
        },

        "labelme_modulated_train": {
            "img_dir": "coco_dataset0227/train",
            "ann_file": "coco_dataset0227/annotations/instances_train.json"
        },
        "labelme_modulated_val": {
            "img_dir": "coco_dataset0227/val",
    ...
    def get(name):
        ...
        if name.endswith('_bg'):
            ...
            args = dict(
                ...
                transforms=None,  # 会在 build.py 中添加
                return_masks=True,
                return_tokens=True,
                is_train=attrs.get("is_train", True),
                tokenizer=None,  # 会在 build.py 中添加
                disable_clip_to_image=False,
                no_mask_for_gold=False,
                max_query_len=256
            )
            return dict(
                factory="ModulatedDataset",
                args=args,
            )
        else:
