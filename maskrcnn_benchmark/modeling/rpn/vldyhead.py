...
class VLDyHead(torch.nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE in ["bert-base-uncased", "bert-base-chinese", "roberta-base"]:
            ...
            if "roberta" in cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE:
                lang_cfg = RobertaConfig.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE)
            else:
            ...
            lang_cfg = None
            raise NotImplementedError(f"Model {cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE} not supported")
