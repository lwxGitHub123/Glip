...
class InstanceColorManager:
    """实例颜色管理器 - 为每个检测到的实例分配不同颜色"""
    ...
    def __init__(self):
        self.instance_colors = {}  # 存储实例ID对应的颜色
        self.color_index = 0
        self.next_instance_id = 0  # 下一个可用的实例ID
    ...
    def assign_color_to_instance(self, instance_id):
        """
        为实例分配颜色
        如果实例已存在，返回之前分配的颜色
        如果是新实例，分配下一个预定义颜色
        ...
        if instance_id not in self.instance_colors:
            ...
            print(f"🎨 为实例 {instance_id} 分配颜色: {color_name}")
    ...
    def _get_color_name(self, color):
        ...
        return color_map.get(color_tuple, f"BGR{color_tuple}")


# 创建全局实例颜色管理器实例
instance_color_manager = InstanceColorManager()


class PatchedGLIPDemo(GLIPDemo):
    """
    修复版的GLIPDemo，添加中文支持和不同实例不同颜色
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.use_instance_colors = True  # 是否使用实例颜色
        self.current_instance_id = 0  # 当前实例ID计数器

        # 获取字体信息
        self.font_name = font_manager.get_font_name()
        print("✅ PatchedGLIPDemo 初始化完成，实例颜色功能已添加")
        print(f"🎨 为每个检测到的实例分配不同颜色")
        if self.font_name:
    ...
    def overlay_entity_names(self, image, predictions, names=None, text_size=1.0, text_pixel=2, text_offset=10,
        ...
        # 为每个检测到的目标绘制
        for i, (box, score, label) in enumerate(zip(boxes, scores, new_labels)):
            ...
            # 获取该实例的颜色
            if self.use_instance_colors:
                # 为每个实例分配一个实例ID
                instance_id = self.current_instance_id
                self.current_instance_id += 1
                color = instance_color_manager.assign_color_to_instance(instance_id)
            else:
                color = (0, 0, 255)  # 默认红色
...
class FixedGLIPInference:
    def __init__(self, weight_path, config_file='configs/pretrain/glip_Swin_T_O365_GoldG.yaml',
                 device=None, min_image_size=480, confidence_threshold=0.5,
                 use_instance_colors=True):
        """
        修复版GLIP推理器
        use_instance_colors: 是否为每个实例使用不同颜色
        """
        # 检查文件是否存在
        ...
        try:
            ...
            # 设置是否使用实例颜色
            self.glip_demo.use_instance_colors = use_instance_colors
            self.glip_demo.font_size = 20

            if use_instance_colors:
                print("🎨 为每个检测到的实例分配不同颜色")
            else:
                print("🎨 所有目标使用默认红色")
    ...
    def predict(self, image_path, captions, threshold=0.5):
        ...
        # 重置颜色管理器（为新图像准备）
        if hasattr(self.glip_demo, 'reset_colors'):
            self.glip_demo.reset_colors()
        ...
        # 为每个实例获取颜色（用于输出显示）- 修复这里的问题
        instance_colors = []
        if self.glip_demo.use_instance_colors and len(boxes) > 0:
            for i in range(len(boxes)):
                color = instance_color_manager.assign_color_to_instance(i)
    ...
    def visualize(self, results, save_path=None):
        ...
        if len(results['boxes']) > 0:
            print(f"\n📊 检测到的目标详细信息:")
            print("-" * 80)
            print(f"{'序号':<6}{'实例ID':<8}{'标签':<15}{'置信度':<10}{'颜色':<15}{'边界框 (x1,y1,x2,y2)':<30}")
            print("-" * 80)
            ...
            for i in range(len(results['boxes'])):
                ...
                # 获取该实例的颜色
                if i < len(results.get('instance_colors', [])):
                    color = results['instance_colors'][i]
                else:
                    color = (0, 0, 255)  # 默认红色
                color_name = instance_color_manager._get_color_name(color)

                # 格式化输出
                box_str = f"({box[0]},{box[1]},{box[2]},{box[3]})" if len(box) == 4 else "无效"
                print(f"{i + 1:<6}{i:<8}{label_name:<15}{score:<10.3f}{color_name:<15}{box_str:<30}")
            ...
            # 打印颜色使用情况
            print("\n🎨 颜色使用情况:")
            unique_colors = set(tuple(color) for color in results.get('instance_colors', []))
            for i, color_tuple in enumerate(unique_colors):
                color_name = instance_color_manager._get_color_name(color_tuple)
                print(f"  - 颜色 {i + 1}: {color_name}")
        else:
            print("📊 未检测到目标")
...
def main():
    """主函数"""
    print("=" * 60)
    print("GLIP 最终修复版推理演示（不同实例不同颜色 + 中文支持）")
    print("=" * 60)

    # 配置参数
    weight_path = 'output/coco_cn_glip_tiny_model_o365_goldg_cc_sbu/model_0022000.pth'
    config_path = 'configs/pretrain/glip_Swin_T_O365_GoldG_coco_cn_predict.yaml'
    ...
    try:
        ...
        inferencer = FixedGLIPInference(
            ...
            use_instance_colors=True  # 启用实例颜色
        )
        ...
        if '猫' in image_path.lower():
            ...
            captions = '黑色的狗'  # '穿军装、拿着帽子、坐在最左边的男人'  # '一只白色的猫，在图片左边，右边有一只黄色的猫。'   # '白色的猫'
        ...
        # 保存结果
        save_path = "result_instance_colors.jpg"
...
def quick_test():
    ...
    try:
        ...
        # 保存结果
        inferencer.visualize(results, save_path='quick_result_instances.jpg')
