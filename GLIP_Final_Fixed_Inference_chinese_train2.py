...
class FixedGLIPInference:
    ...
    def visualize(self, results, save_path=None):
        ...
        if len(results['boxes']) > 0:
            print(f"\n📊 检测到的目标详细信息:")
            print("-" * 60)
            print(f"{'序号':<6}{'标签':<20}{'置信度':<10}{'边界框 (x1,y1,x2,y2)':<30}")
            print("-" * 60)
            ...
            for i in range(len(results['boxes'])):
                ...
                print(f"{i + 1:<6}{label_name:<20}{score:<10.3f}{box_str:<30}")
...
def main():
    ...
    try:
        ...
        # ==================== 方法2：使用多个独立提示 ====================
        # 根据图像内容调整检测类别 - 使用多个独立提示词
        if '猫' in image_path.lower():
            # 方法2：使用多个独立的提示词（用句号分隔）
            captions = '白色的猫. 黄色的猫. 左边的猫. 右边的猫. 猫. 狸花猫. 小花猫'
        else:
            # 默认使用多个独立提示
            captions = '狗. 白色的狗. 黄色的狗. 黑色的狗. 草地上的狗. 金毛狗'

        # 或者直接指定要检测的多个独立类别
        # captions = '白色的猫. 黄色的猫. 左边的猫. 右边的猫. 猫'

        print(f"🎯 使用多个独立提示词（方法2）")
        print(f"📋 检测类别列表: {captions}")
        ...
        # 额外分析：根据位置解析猫的位置
        if len(results['boxes']) > 0 and len(results.get('entity_names', [])) > 0:
            print(f"\n🔍 详细位置分析:")
            image = cv2.imread(image_path)
            height, width = image.shape[:2]

            for i, (entity_name, score, box) in enumerate(zip(results['entity_names'],
                                                              results['scores'],
                                                              results['boxes'])):
                ...
                print(f"目标 {i + 1}: {entity_name} (置信度: {score:.3f}) - 位于图像{position}")
...
def quick_test():
    ...
    try:
        ...
        # 运行推理 - 使用多个独立提示词（方法2）
        captions = '白色的猫. 黄色的猫. 左边的猫. 右边的猫. 猫'
        print(f"🎯 使用多个独立提示词（方法2）: {captions}")
        ...
        if len(results['boxes']) > 0:
            ...
            for i, (entity_name, score, box) in enumerate(zip(results['entity_names'],
                ...
                print(f"  目标 {i + 1}: {entity_name:<15} (置信度: {score:.3f}) - {position}")
